...
// 호출자는 "지시 + 대화 → 이 스키마의 객체"만 안다. 모델·프로토콜·파싱은 모른다.
// 출력 형식은 OpenRouter response_format(json_object)이 강제하므로, 모델은 JSON 하나만 낸다.
...
// 스트리밍 표시용 — 출력 JSON의 message 필드가 자라는 만큼 증분 텍스트를 흘린다(없으면 끝까지 모아 반환만).
...
export interface LlmClient {
  ...
  // onDelta가 주어지면 응답 JSON의 message 필드 증분을 흘린다(SSE 전송용).
  generate: <T>(
    input: {
      system: string;
      messages: LlmMessage[];
