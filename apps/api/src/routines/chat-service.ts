...
export interface RoutineChatService {
  reply: (history: RoutineChatRequestDto['history']) => Promise<RoutineChatResultDto>;
}
...
// 출력은 항상 JSON 하나 — response_format(json_object)와 RoutineChatResultDto가 이중으로 강제.
...
출력은 반드시 아래 두 형태 중 하나의 JSON 객체로만 응답한다(코드블록·설명 텍스트 없이 JSON 그 자체):
